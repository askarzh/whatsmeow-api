...
func (s *ContactStore) Get(ctx context.Context, jid string) (store.Contact, error) {
	row := s.db.QueryRowContext(ctx, `SELECT `+contactColumns+` FROM contacts WHERE jid = $1`, jid)
...
func (s *ContactStore) Search(ctx context.Context, query string, limit int) ([]store.Contact, error) {
	...
	rows, err := s.db.QueryContext(ctx, `
		...
		WHERE lower(coalesce(push_name, '')) LIKE $1
		   OR lower(coalesce(full_name, '')) LIKE $2
		   OR lower(coalesce(business_name, '')) LIKE $3
		ORDER BY jid ASC
		LIMIT $4
	`, pat, pat, pat, limit)
	if err != nil {
